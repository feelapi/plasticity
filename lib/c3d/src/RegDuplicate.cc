...
Napi::Value RegDuplicate::IsReg(const Napi::CallbackInfo &info)
{
    ...
    bool _result = _underlying->IsReg(srcItem, cpyItem
...
Napi::Value RegDuplicate::IsReg_async(const Napi::CallbackInfo &info)
{
    ...
    RegDuplicate_IsReg_AsyncWorker *asyncWorker = new RegDuplicate_IsReg_AsyncWorker(_underlying, deferred,
...
Napi::Value RegDuplicate::SetReg(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 || !((info[1].IsObject() && info[1].ToObject().InstanceOf(RefItem::GetConstructor(env)))))
    {
    ...
    _underlying->SetReg(srcItem, cpyItem
...
Napi::Value RegDuplicate::SetReg_async(const Napi::CallbackInfo &info)
{
    ...
    RegDuplicate_SetReg_AsyncWorker *asyncWorker = new RegDuplicate_SetReg_AsyncWorker(_underlying, deferred,
...
RegDuplicate_IsReg_AsyncWorker::RegDuplicate_IsReg_AsyncWorker(MbRegDuplicate *_underlying,
                                                               Napi::Promise::Deferred const &d,
                                                               const MbRefItem *srcItem)
    : _underlying(_underlying), PromiseWorker(d), srcItem(srcItem){};
...
RegDuplicate_SetReg_AsyncWorker::RegDuplicate_SetReg_AsyncWorker(MbRegDuplicate *_underlying,
                                                                 Napi::Promise::Deferred const &d,
                                                                 const MbRefItem *srcItem, MbRefItem *cpyItem)
    : _underlying(_underlying), PromiseWorker(d), srcItem(srcItem), cpyItem(cpyItem){};
