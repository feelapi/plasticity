...
if (!(info[0].IsNull() || info[0].IsUndefined())) {
 ...
 MbResultType  _result = ::CreateConvexPolyhedron
(
        points
,        result
...
if (!(info[0].IsNull() || info[0].IsUndefined())) {
    ...
    for (size_t i = 0; i < points_.Length(); i++) {
        ...
        ActionMesh_CreateConvexPolyhedron_AsyncWorker* asyncWorker = new ActionMesh_CreateConvexPolyhedron_AsyncWorker(
            deferred
,
                                *points                        );
        asyncWorker->Queue();
...
,                        result
    ...
    void ActionMesh_CreateConvexPolyhedron_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
