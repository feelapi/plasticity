...
const char *Error::GetSolidErrorResId(uint solidError)
{
    ...
    case rt_NoIntersectSection: // Образующий контур не пересекает сечения (для операции построения тела по сечениям)
        return RT_NO_INTERSECT_SECTION; // Образующий контур не пересекает сечения (для операции построения тела по
                                        // сечениям)
    ...
    // Типы ошибок
    case rt_NoSequenceCurveAndSections: // Не последовательное расположение сечений вдоль кривой (для операции
                                        // построения тела по сечениям)
        return RT_NO_SEQUENCE_CURVE_AND_SECTIONS; // Не последовательное расположение сечений вдоль кривой (для операции
                                                  // построения тела по сечениям)
    ...
    case rt_SplitWireNotIntersectFace: // Контур не пересекает ни одну из граней или совпадает с кромкой
                                       // грани(MA 17.07.2001)
