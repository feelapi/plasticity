...
Napi::Object ActionRegion::Init(Napi::Env env, Napi::Object exports) {
    ...
    object.Set("CreateBooleanResultRegions_async", Napi::Function::New<&ActionRegion::CreateBooleanResultRegions_async>(env));
...
if (!(info[0].IsNull() || info[0].IsUndefined())) {
    ...
    for (size_t i = 0; i < contours_.Length(); i++) {
        ...
        ActionRegion_GetCorrectRegions_AsyncWorker* asyncWorker = new ActionRegion_GetCorrectRegions_AsyncWorker(
            deferred
,
                                *contours                ,
...
if (!(info[0].IsNull() || info[0].IsUndefined())) {
    ...
    for (size_t i = 0; i < contours_.Length(); i++) {
        ...
        ActionRegion_MakeRegions_AsyncWorker* asyncWorker = new ActionRegion_MakeRegions_AsyncWorker(
            deferred
,
                                *contours                ,
        ...
        Napi::Value ActionRegion::CreateBooleanResultRegions(const Napi::CallbackInfo& info) {
            Napi::Env env = info.Env();
                        if (info.Length() == 0 || !(
              (info[0].IsObject() && info[0].ToObject().InstanceOf(Region::GetConstructor(env)))
)) {
                    Napi::Error::New(env, "Region region1 is required.").ThrowAsJavaScriptException();
                    return env.Undefined();
            }
            if (info.Length() == 1 || !(
              (info[1].IsObject() && info[1].ToObject().InstanceOf(Region::GetConstructor(env)))
)) {
 ...
 bool  _result = ::CreateBooleanResultRegions
(
        region1
,        region2
,        operParams
...
if (_result) {
    ...
    Napi::Value ActionRegion::CreateBooleanResultRegions_async(const Napi::CallbackInfo& info) {
        ...
        ActionRegion_CreateBooleanResultRegions_AsyncWorker* asyncWorker = new ActionRegion_CreateBooleanResultRegions_AsyncWorker(
            deferred
,
...
,                        *regions
    ...
    void ActionRegion_GetCorrectRegions_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
    }
    ActionRegion_MakeRegions_AsyncWorker::ActionRegion_MakeRegions_AsyncWorker(
        Napi::Promise::Deferred const &d
,
                         RPArray<MbContour> & contours,
        ...
        :PromiseWorker(d),
                        contours(contours)
,
                        useSelfIntCntrs(useSelfIntCntrs)
,
                        sameContours(sameContours)
{};
...
,                        resInfo
    ...
    void ActionRegion_CreateBooleanResultRegions_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
