...
Napi::Value TriFace::CalculateGrid(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 || !((info[1].IsObject() && info[1].ToObject().InstanceOf(StepData::GetConstructor(env)))))
    {
        Napi::Error::New(env, "StepData stepData is required.").ThrowAsJavaScriptException();
        return env.Undefined();
    }
    if (info.Length() == 2 || !((info[2].IsObject() && info[2].ToObject().InstanceOf(Grid::GetConstructor(env)))))
    {
    ...
    ::CalculateGrid(face, stepData, grid, info.Length() == 3 || info[3].IsNull() ? true : dualSeams,
...
Napi::Value TriFace::CalculateGrid_async(const Napi::CallbackInfo &info)
{
    ...
    TriFace_CalculateGrid_AsyncWorker *asyncWorker = new TriFace_CalculateGrid_AsyncWorker(deferred,
...
TriFace_CalculateGrid_AsyncWorker::TriFace_CalculateGrid_AsyncWorker(Napi::Promise::Deferred const &d,
                                                                     const MbFace &face, const MbStepData &stepData,
                                                                     MbGrid &grid, bool dualSeams, bool quad, bool fair)
    : PromiseWorker(d), face(face), stepData(stepData), grid(grid), dualSeams(dualSeams), quad(quad), fair(fair){};

void TriFace_CalculateGrid_AsyncWorker::Execute()
{
    EnterParallelRegion();

    ::CalculateGrid(face, stepData, grid, dualSeams, quad, fair);
