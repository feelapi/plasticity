...
Napi::Function Vector3D::GetConstructor(Napi::Env env) {
 ...
 bool  _result = _underlying->Colinear
(
        other
,        info.Length() == 1 || info[1].IsNull() ? Math::angleRegion : eps
    ...
    Napi::Value Vector3D::Colinear_async(const Napi::CallbackInfo& info) {
        ...
        Vector3D_Colinear_AsyncWorker* asyncWorker = new Vector3D_Colinear_AsyncWorker(
...
(
    ...
    Napi::Value Vector3D::Invert_async(const Napi::CallbackInfo& info) {
        ...
        Vector3D_Invert_AsyncWorker* asyncWorker = new Vector3D_Invert_AsyncWorker(
_underlying,            deferred
        );
        asyncWorker->Queue();
        return deferred.Promise();
...
void Vector3D::SetValue_z(const Napi::CallbackInfo &info, const Napi::Value &value) {
    ...
    Vector3D_Colinear_AsyncWorker::Vector3D_Colinear_AsyncWorker(
MbVector3D * _underlying,        Napi::Promise::Deferred const &d
,
...
,                        eps
    ...
    Vector3D_Invert_AsyncWorker::Vector3D_Invert_AsyncWorker(
MbVector3D * _underlying,        Napi::Promise::Deferred const &d
    )
        :_underlying(_underlying),PromiseWorker(d){};
