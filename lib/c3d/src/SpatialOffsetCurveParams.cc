...
SpatialOffsetCurveParams::SpatialOffsetCurveParams(const Napi::CallbackInfo& info) : Napi::ObjectWrap<SpatialOffsetCurveParams>(info) {
 ...
 &&         ((
                info[1].IsObject() &&
                    info[1].ToObject().InstanceOf(SNameMaker::GetConstructor(env))
        )   
)
