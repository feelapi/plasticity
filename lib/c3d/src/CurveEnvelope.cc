...
if (!(info[1].IsNull() || info[1].IsUndefined())) {
    ...
    for (size_t i = 0; i < fromCurve_.Length(); i++) {
        ...
        CurveEnvelope_IntersectWithAll_AsyncWorker* asyncWorker = new CurveEnvelope_IntersectWithAll_AsyncWorker(
            deferred
,
...
if (!(info[2].IsNull() || info[2].IsUndefined())) {
    ...
    for (size_t i = 0; i < cross_.Length(); i++) {
        ...
        CurveEnvelope_SortCrossPoints_AsyncWorker* asyncWorker = new CurveEnvelope_SortCrossPoints_AsyncWorker(
            deferred
,
    ...
    CurveEnvelope_IntersectWithAll_AsyncWorker::CurveEnvelope_IntersectWithAll_AsyncWorker(
...
,                        self
    ...
    void CurveEnvelope_IntersectWithAll_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
    }
    CurveEnvelope_SortCrossPoints_AsyncWorker::CurveEnvelope_SortCrossPoints_AsyncWorker(
        Napi::Promise::Deferred const &d
,
                         double  tProj,
                        const MbCurve * selectCurve,
...
,                        *crossRight
    ...
    void CurveEnvelope_SortCrossPoints_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
