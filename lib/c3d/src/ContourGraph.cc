...
(
        curveList
,        *contours
,        info.Length() == 1 || info[1].IsNull() ? METRIC_ACCURACY : accuracy
...
if (!(info[0].IsNull() || info[0].IsUndefined())) {
    ...
    for (size_t i = 0; i < curveList_.Length(); i++) {
        ...
        ContourGraph_OuterContoursBuilder_AsyncWorker* asyncWorker = new ContourGraph_OuterContoursBuilder_AsyncWorker(
            deferred
,
                                *curveList                ,
 ...
 bool  _result = ::RemoveContourGaps
(
        contour
,        accuracy
,        canInsert
,        canReplace
    ...
    Napi::Value ContourGraph::RemoveContourGaps_async(const Napi::CallbackInfo& info) {
        ...
        ContourGraph_RemoveContourGaps_AsyncWorker* asyncWorker = new ContourGraph_RemoveContourGaps_AsyncWorker(
            deferred
,
...
,                        progInd
    ...
    void ContourGraph_OuterContoursBuilder_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
    }
    ContourGraph_RemoveContourGaps_AsyncWorker::ContourGraph_RemoveContourGaps_AsyncWorker(
        Napi::Promise::Deferred const &d
,
                         MbContour & contour,
                         double  accuracy,
...
,                        canReplace
    ...
    void ContourGraph_RemoveContourGaps_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
