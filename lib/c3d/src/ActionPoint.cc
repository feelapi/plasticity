...
Napi::Object ActionPoint::Init(Napi::Env env, Napi::Object exports) {
    ...
    object.Set("CurveCurveIntersection2D_async", Napi::Function::New<&ActionPoint::CurveCurveIntersection2D_async>(env));
    object.Set("CurveCurveIntersection3D", Napi::Function::New<&ActionPoint::CurveCurveIntersection3D>(env));
    object.Set("CurveCurveIntersection3D_async", Napi::Function::New<&ActionPoint::CurveCurveIntersection3D_async>(env));
    ...
    return exports;
        ...
        Napi::Value ActionPoint::LineLineNearestPoints(const Napi::CallbackInfo& info) {
            Napi::Env env = info.Env();
                        if (info.Length() == 0 || !(
              (info[0].IsObject() && info[0].ToObject().InstanceOf(Line3D::GetConstructor(env)))
)) {
                    Napi::Error::New(env, "Line3D line1 is required.").ThrowAsJavaScriptException();
                    return env.Undefined();
            }
            if (info.Length() == 1 || !(
              (info[1].IsObject() && info[1].ToObject().InstanceOf(Line3D::GetConstructor(env)))
)) {
 ...
 double  _result = ::LineLineNearestPoints
(
        line1
,        line2
,        p1
,        p2
    ...
    Napi::Value ActionPoint::LineLineNearestPoints_async(const Napi::CallbackInfo& info) {
        ...
        ActionPoint_LineLineNearestPoints_AsyncWorker* asyncWorker = new ActionPoint_LineLineNearestPoints_AsyncWorker(
            deferred
,
 ...
 ptrdiff_t  count = ::CurveCurveIntersection
(
        curve1
,        curve2
,        *result1
...
,        info.Length() == 5 || info[5].IsNull() ? true : allowInaccuracy
    ...
    Napi::Value ActionPoint::CurveCurveIntersection2D_async(const Napi::CallbackInfo& info) {
        ...
        ActionPoint_CurveCurveIntersection2D_AsyncWorker* asyncWorker = new ActionPoint_CurveCurveIntersection2D_AsyncWorker(
            deferred
,
...
,        mEps
    ...
    Napi::Value ActionPoint::CurveCurveIntersection3D_async(const Napi::CallbackInfo& info) {
        ...
        ActionPoint_CurveCurveIntersection3D_AsyncWorker* asyncWorker = new ActionPoint_CurveCurveIntersection3D_AsyncWorker(
            deferred
,
 ...
 ptrdiff_t  count = ::CurveCurveCrossing
(
        curve1
,        curve2
,        *result1
...
,        info.Length() == 2 || info[2].IsNull() ? Math::metricRegion : mEps
    ...
    Napi::Value ActionPoint::CurveCurveCrossing_async(const Napi::CallbackInfo& info) {
        ...
        ActionPoint_CurveCurveCrossing_AsyncWorker* asyncWorker = new ActionPoint_CurveCurveCrossing_AsyncWorker(
            deferred
,
...
,                        p2
    ...
    void ActionPoint_LineLineNearestPoints_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
...
,
                        yEpsilon(yEpsilon)
,
                        touchInclude(touchInclude)
,
                        allowInaccuracy(allowInaccuracy)
{};
...
,                        allowInaccuracy
    ...
    void ActionPoint_CurveCurveIntersection2D_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
...
,                        mEps
    ...
    void ActionPoint_CurveCurveIntersection3D_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
    }
    ActionPoint_CurveCurveCrossing_AsyncWorker::ActionPoint_CurveCurveCrossing_AsyncWorker(
        Napi::Promise::Deferred const &d
,
                        const MbCurve3D & curve1,
                        const MbCurve3D & curve2,
...
,                        mEps
    ...
    void ActionPoint_CurveCurveCrossing_AsyncWorker::Reject(Napi::Promise::Deferred const &deferred, Napi::Error const & error) {
        Napi::Env env = deferred.Env();
        error.Value().Set("code", Napi::Number::New(env, this->resultType));
        error.Value()["isC3dError"] = true;
        deferred.Reject(error.Value());
