...
EdgeFunction::EdgeFunction(const Napi::CallbackInfo& info) : Napi::ObjectWrap<EdgeFunction>(info) {
    ...
    if (info.Length() == 1 && info[0].IsString() && info[0].ToString().Utf8Value() == "__skip_js_init__") return;
        if (info.Length() == 2&&                ((
                info[0].IsObject() &&
                    info[0].ToObject().InstanceOf(CurveEdge::GetConstructor(env))
        )   
)
 &&         ((
                info[1].IsObject() &&
...
(
    ...
    Napi::Value EdgeFunction::Edge_async(const Napi::CallbackInfo& info) {
        ...
        EdgeFunction_Edge_AsyncWorker* asyncWorker = new EdgeFunction_Edge_AsyncWorker(
...
(
    ...
    Napi::Value EdgeFunction::Function_async(const Napi::CallbackInfo& info) {
        ...
        EdgeFunction_Function_AsyncWorker* asyncWorker = new EdgeFunction_Function_AsyncWorker(
_underlying,            deferred
        );
        asyncWorker->Queue();
        return deferred.Promise();
    ...
    EdgeFunction_Edge_AsyncWorker::EdgeFunction_Edge_AsyncWorker(
MbEdgeFunction * _underlying,        Napi::Promise::Deferred const &d
    ...
    EdgeFunction_Function_AsyncWorker::EdgeFunction_Function_AsyncWorker(
MbEdgeFunction * _underlying,        Napi::Promise::Deferred const &d
    )
        :_underlying(_underlying),PromiseWorker(d){};
