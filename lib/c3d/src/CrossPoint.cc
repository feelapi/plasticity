...
Napi::Object CrossPoint::Init(const Napi::Env env, Napi::Object exports) {
    Napi::Function func = DefineClass(env, "CrossPoint", {
...
CrossPoint::CrossPoint(const Napi::CallbackInfo& info) : Napi::ObjectWrap<CrossPoint>(info) {
 ...
 &&         ((
                info[1].IsObject() &&
                    info[1].ToObject().InstanceOf(PointOnCurve::GetConstructor(env))
        )   
)
 &&         ((
                info[2].IsObject() &&
                    info[2].ToObject().InstanceOf(PointOnCurve::GetConstructor(env))
        )   
)
