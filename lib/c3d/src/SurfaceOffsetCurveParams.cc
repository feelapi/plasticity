...
SurfaceOffsetCurveParams::SurfaceOffsetCurveParams(const Napi::CallbackInfo& info) : Napi::ObjectWrap<SurfaceOffsetCurveParams>(info) {
    Napi::Env env = info.Env();
    if (info.Length() == 1 && info[0].IsString() && info[0].ToString().Utf8Value() == "__skip_js_init__") return;
        if (info.Length() == 4&&                ((
                info[0].IsObject() &&
                    info[0].ToObject().InstanceOf(Face::GetConstructor(env))
        )   
)
 &&         ((
                info[1].IsObject() &&
                    info[1].ToObject().InstanceOf(Axis3D::GetConstructor(env))
        )   
)
 &&         ((
                info[2].IsNumber()
 ...
 &&         ((
                info[3].IsObject() &&
                    info[3].ToObject().InstanceOf(SNameMaker::GetConstructor(env))
        )   
)
