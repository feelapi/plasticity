...
Napi::Value Conversion::ImportFromFile(const Napi::CallbackInfo &info)
{
    ...
    MbeConvResType result = c3d::ImportFromFile(*model, fileName, info.Length() == 1 || info[1].IsNull() ? NULL : prop,
...
Napi::Value Conversion::ImportFromFile_async(const Napi::CallbackInfo &info)
{
    ...
    Conversion_ImportFromFile_AsyncWorker *asyncWorker = new Conversion_ImportFromFile_AsyncWorker(deferred,
...
{
    Napi::Env env = info.Env();
    if (info.Length() == 0 || !((info[0].IsObject() && info[0].ToObject().InstanceOf(Model::GetConstructor(env)))))
    {
    ...
    MbeConvResType _result = c3d::ExportIntoFile(model, fileName, info.Length() == 2 || info[2].IsNull() ? NULL : prop,
...
Napi::Value Conversion::ExportIntoFile_async(const Napi::CallbackInfo &info)
{
    ...
    Conversion_ExportIntoFile_AsyncWorker *asyncWorker = new Conversion_ExportIntoFile_AsyncWorker(deferred,
...
Conversion_ImportFromFile_AsyncWorker::Conversion_ImportFromFile_AsyncWorker(Napi::Promise::Deferred const &d,
                                                                             const c3d::path_string &fileName,
                                                                             ConvConvertorProperty3D *prop,
                                                                             ProgressIndicator *indicator)
    : PromiseWorker(d), fileName(fileName), prop(prop), indicator(indicator){};
...
void Conversion_ImportFromFile_AsyncWorker::Execute()
{
    ...
    MbeConvResType result = ::c3d::ImportFromFile(*model, fileName, prop, indicator);
...
Conversion_ExportIntoFile_AsyncWorker::Conversion_ExportIntoFile_AsyncWorker(Napi::Promise::Deferred const &d,
                                                                             MbModel &model,
                                                                             const c3d::path_string &fileName,
                                                                             ConvConvertorProperty3D *prop,
                                                                             ProgressIndicator *indicator)
    : PromiseWorker(d), model(model), fileName(fileName), prop(prop), indicator(indicator){};

void Conversion_ExportIntoFile_AsyncWorker::Execute()
{
    EnterParallelRegion();

    MbeConvResType _result = ::c3d::ExportIntoFile(model, fileName, prop, indicator);
