...
_SolidPool::_SolidPool(const Napi::CallbackInfo& info) : Napi::ObjectWrap<_SolidPool>(info) {
    ...
    if (info.Length() == 1 && info[0].IsString() && info[0].ToString().Utf8Value() == "__skip_js_init__") return;
        if (info.Length() == 1&&                ((
                info[0].IsObject() &&
                    info[0].ToObject().InstanceOf(Solid::GetConstructor(env))
        )   
)
...
(
    ...
    Napi::Value _SolidPool::Alloc_async(const Napi::CallbackInfo& info) {
        ...
        _SolidPool_Alloc_AsyncWorker* asyncWorker = new _SolidPool_Alloc_AsyncWorker(
...
(
    ...
    Napi::Value _SolidPool::Pop_async(const Napi::CallbackInfo& info) {
        ...
        _SolidPool_Pop_AsyncWorker* asyncWorker = new _SolidPool_Pop_AsyncWorker(
_underlying,            deferred
        );
        asyncWorker->Queue();
        return deferred.Promise();
...
(
    ...
    Napi::Value _SolidPool::Count_async(const Napi::CallbackInfo& info) {
        ...
        _SolidPool_Count_AsyncWorker* asyncWorker = new _SolidPool_Count_AsyncWorker(
_underlying,            deferred
        );
        asyncWorker->Queue();
        return deferred.Promise();
    ...
    _SolidPool_Alloc_AsyncWorker::_SolidPool_Alloc_AsyncWorker(
SolidPool * _underlying,        Napi::Promise::Deferred const &d
,
                         size_t  n    )
...
{};
    ...
    _SolidPool_Pop_AsyncWorker::_SolidPool_Pop_AsyncWorker(
SolidPool * _underlying,        Napi::Promise::Deferred const &d
    )
        :_underlying(_underlying),PromiseWorker(d){};
    ...
    _SolidPool_Count_AsyncWorker::_SolidPool_Count_AsyncWorker(
SolidPool * _underlying,        Napi::Promise::Deferred const &d
    )
        :_underlying(_underlying),PromiseWorker(d){};
