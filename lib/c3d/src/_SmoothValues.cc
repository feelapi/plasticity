...
Napi::Object _SmoothValues::Init(const Napi::Env env, Napi::Object exports)
{
    Napi::Function func = DefineClass(
        env, "SmoothValues",
        {
            ...
            InstanceAccessor<&_SmoothValues::GetValue_smoothCorner, &_SmoothValues::SetValue_smoothCorner>(
                "smoothCorner"),
...
_SmoothValues::_SmoothValues(const Napi::CallbackInfo &info) : Napi::ObjectWrap<_SmoothValues>(info)
{
    ...
    else if (info.Length() == 10 && ((info[0].IsNumber())) && ((info[1].IsNumber())) && ((info[2].IsNumber())) &&
             ((info[3].IsNumber())) && ((info[4].IsBoolean())) && ((info[5].IsNumber())) && ((info[6].IsBoolean())) &&
             ((info[7].IsBoolean())) && ((info[8].IsBoolean())) && ((info[9].IsBoolean()))
...
{
    Napi::Env env = info.Env();
    const SmoothValues::CornerForm smoothCorner =
        static_cast<SmoothValues::CornerForm>(info[0].ToNumber().Uint32Value());
